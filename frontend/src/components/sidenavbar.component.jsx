...
const SideNav = () => {
    let { userAuth: { access_token, new_notification_available, language } } = useContext(UserContext); // Assume language is part of userAuth
            ...
            <section className="relative flex gap-10 py-0 m-0 max-md:flex-col">
                ...
                <div className="sticky top-[80px] z-30">
                    ...
                    <div className={"min-w-[200px] h-[calc(100vh-80px-60px)] md:h-cover md:sticky top-24 overflow-y-auto p-6 md:pr-0 md:border-grey md:border-r absolute max-md:top-[64px] bg-white max-md:w-[calc(100%+80px)] max-md:px-16 max-md:-ml-7 duration-500 " + (!showSideNav ? "max-md:opacity-0 max-md:pointer-events-none" : "opacity-100 pointer-events-auto")}>
                        ...
                        <h1 className="text-xl text-dark-grey mb-3">{translations.dashboard}</h1>
                        ...
                        <NavLink to="/dashboard/notifications" onClick={(e) => setPageState(e.target.innerText)} className="sidebar-link">
                            <div className="relative">
                                <i className="fi fi-rr-bell"></i>
                                {
                                    ...
                                    <span className="bg-red w-4 h-4 rounded-full absolute z-10 -top-1 -right-2 flex items-center justify-center text-white text-[10px] font-bold">{new_notification_available > 99 ? '99+' : new_notification_available}</span> : ""
                                }
                            </div>
                        ...
                        <h1 className="text-xl text-dark-grey mt-20 mb-3">{translations.settings}</h1>
                        <hr className="border-grey -ml-6 mb-8 mr-6" />
